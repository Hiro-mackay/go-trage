...
func (c *Candle) Create() error {
	...
	if err != nil {
		log.Printf("Error in Candle Create : %v", err)
	}
	return err
}
...
func (c *Candle) Save() error {
	...
	if err != nil {
		log.Printf("Error in Candle Save : %v", err)
	}
	return err
}
...
func CreateCandleWithDuration(ticker bitflyer.Ticker, productCode string, duration time.Duration) bool {
	...
	if currentCandle == nil {
		candle := NewCandle(
			productCode, duration, ticker.TruncateDateTime(duration), price, price, price, price, ticker.Volume,
		)
		candle.Create()
		return true
	}
